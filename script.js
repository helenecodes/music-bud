class MusicBud {
    ...
    createTrackCard(track) {
        ...
        const tempo = track.audioFeatures?.tempo ? Math.round(track.audioFeatures.tempo) + ' BPM' : 'N/A';
    ...
    showTrackModal(track) {
        ...
        document.getElementById('modal-track-image').src = track.album?.images?.[1]?.url || track.images?.[1]?.url || '';
        document.getElementById('modal-track-name').textContent = track.name;
        document.getElementById('modal-track-artist').textContent = track.artists?.map(a => a.name).join(', ') || 'Unknown Artist';
        document.getElementById('modal-track-album').textContent = track.album?.name || 'Unknown Album';
        
        // Populate audio features
        document.getElementById('modal-tempo').textContent = features.tempo ? Math.round(features.tempo) + ' BPM' : 'N/A';
        document.getElementById('modal-key').textContent = this.getKeyName(features.key, features.mode);
        document.getElementById('modal-time-signature').textContent = features.time_signature ? features.time_signature + '/4' : 'N/A';
        document.getElementById('modal-release-date').textContent = track.album?.release_date || 'Unknown';
        document.getElementById('modal-duration').textContent = this.formatDuration(track.duration_ms);
        
        document.getElementById('modal-energy').textContent = features.energy ? Math.round(features.energy * 100) + '%' : 'N/A';
        document.getElementById('modal-danceability').textContent = features.danceability ? Math.round(features.danceability * 100) + '%' : 'N/A';
        document.getElementById('modal-valence').textContent = features.valence ? Math.round(features.valence * 100) + '%' : 'N/A';
        document.getElementById('modal-acousticness').textContent = features.acousticness ? Math.round(features.acousticness * 100) + '%' : 'N/A';
        document.getElementById('modal-instrumentalness').textContent = features.instrumentalness ? Math.round(features.instrumentalness * 100) + '%' : 'N/A';
        document.getElementById('modal-liveness').textContent = features.liveness ? Math.round(features.liveness * 100) + '%' : 'N/A';
        document.getElementById('modal-speechiness').textContent = features.speechiness ? Math.round(features.speechiness * 100) + '%' : 'N/A';
